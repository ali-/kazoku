...
router.get('/:uuid', (request, response, next) => {
	...
	db.query(query_photo)
		.then(photos => {
			...
			const query_album = `SELECT id, uuid, user_id, private FROM albums WHERE id = '${photos.rows[0].album_id}'`;
			return Promise.all([photos, db.query(query_album)]);
		})
		.then(([photos, albums]) => {
...
router.post('/:uuid/comment', (request, response, next) => {
	...
	Promise.all([db.query(query_album), db.query(query_photo)])
		.then(([albums, photos]) => {
	...
	db.query(query_album)
		.then(albums => {
			...
			image
				.metadata()
				.then(({ exif }) => {
					console.log(exif_reader(exif).exif);
					const date_exif = (exif == null || exif_reader(exif).exif == undefined) ? new Date() : new Date(exif_reader(exif).exif.DateTimeOriginal);
